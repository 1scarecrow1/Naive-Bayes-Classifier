...
$$\begin{equation}
S =
\begin{cases}0,  \  \text{if the document is spam} \\ 
1, \   \text{if the document is not spam}\end{cases}
\end{equation}$$
...
$$P (S = 1) ∏_{i = 1}^n P (X = xi|S = 1) > P (S = 0) ∏_{i = 1}^n P (X = xi|S = 0)$$

Hence
$$P (S = 1) = 1 − ̂P (S = 0) =  \frac{\text{total no. of spam documents}} {\text{total no. of documents}}$$
